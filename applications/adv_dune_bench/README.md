...
```bash
./holohub build adv_dune_bench --build-args="--target rivermax" --configure-args="-D ANO_MGR:STRING=rivermax"
```

### Run Instructions

First, edit the `adv_dune_bench_default_tx_rx.yaml` file to set the `eth_dst_addr` and `address` fields (fields with the `<>` placeholders) based on your system interfaces.

Then:

```bash
./holohub run adv_dune_bench --docker-opts "-u root --privileged"
```

To run with a different configuration file than the default `adv_dune_bench_default_tx_rx`, you need to call the application explicitly:

```bash
# Start the container interactively
./holohub run-container adv_dune_bench \
  --docker-opts="-u root --privileged -w /workspace/holohub/build/adv_dune_bench/applications/adv_dune_bench/cpp"

# Run with GPUNetIO configuration
./adv_dune_bench adv_dune_bench_gpunetio_tx_rx.yaml

# Run with Rivermax configuration
./adv_dune_bench adv_dune_bench_rmax_rx.yaml
```
...
./holohub run-container adv_dune_bench \
  ...
  --docker-opts="\
    ...
    -w /workspace/holohub/build/adv_dune_bench/applications/adv_dune_bench/cpp \
  "
...
```bash
./holohub build adv_dune_bench --configure-args="-D BUILD_TESTING:BOOL=ON"
```
...
# One-shot...
./holohub run-container adv_dune_bench \
  --docker-opts "-u 0 --privileged -w /workspace/holohub/build/adv_dune_bench/" \
  -- ctest
